...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    data_dict = {
        # 'train' : '/root/jupyter/personal/dataset/mobis',
        # 'val' : '/root/jupyter/personal/dataset/mobis',
        'train' : '/root/jupyter/personal/dataset/rg3_17',
        'val' : '/root/jupyter/personal/dataset/rg3_17',
        'nc' : 1,
        'names' : ['BOX'],
    }
    ...
    if opt.adam:
        ...
        print("opt.lr : ", opt.lr)
        optimizer = Adam(g0, lr=opt.lr, betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
    else:
        # optimizer = SGD(g0, lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)
        optimizer = SGD(g0, lr=opt.lr, momentum=hyp['momentum'], nesterov=True)
    ...
    # Trainloader
    train_loader, dataset = create_dataloader(train_path, 'train', imgsz, batch_size // WORLD_SIZE, gs, names, single_cls,
    ...
    # Process 0
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, 'val', imgsz, batch_size // WORLD_SIZE * 2, gs, names, single_cls,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                last = w / ("%s epoch.pt"%(epoch))
                # Save last, best and delete
                if best_fitness == fi:
                    best = w / ("best(%s epoch).pt"%(epoch))
